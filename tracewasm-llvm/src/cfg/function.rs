...
impl GlobalId<DefinedFunc> {
    ...
    pub fn add_basic_block<T: Into<String>>(
        &self,
        name: T,
        ctx: &mut Context,
    ) -> Result<BasicBlockId, ContextError> {
        let name_id: StrId = ctx.str_interner.intern(name.into()).into();
...
mod tests {
    ...
    fn nth_param_is_none_past_the_end() {
        ...
        assert!(one_param.nth_param(0, &builder).is_some());
        assert!(
            one_param.nth_param(1, &builder).is_none(),
            "one past the end"
        );
        assert!(one_param.nth_param(99, &builder).is_none());
        ...
        assert!(
            no_params.nth_param(0, &builder).is_none(),
    ...
    fn return_ty_reports_the_declared_result() {
        ...
        assert_eq!(returns_i32.return_ty(&builder), i32_ty);
        assert_eq!(returns_void.return_ty(&builder), void_ty);

        assert_eq!(
            builder.display(returns_i32.return_ty(&builder)).to_string(),
            "i32"
        );
        assert_eq!(
            builder
                .display(returns_void.return_ty(&builder))
                .to_string(),
            "void"
        );
    }
    ...
    fn only_the_first_block_of_a_function_is_the_entry() {
        ...
        assert!(builder.blocks.get(entry.raw()).unwrap().is_first);
        assert!(!builder.blocks.get(body.raw()).unwrap().is_first);
        ...
        assert!(builder.blocks.get(g_entry.raw()).unwrap().is_first);
    }
    ...
    fn a_block_and_its_function_agree() {
        ...
        assert_eq!(
            builder.blocks.get(entry.raw()).unwrap().func_id.raw(),
            f.raw()
        );
        assert_eq!(builder.funcs.get(f.raw()).unwrap().blocks, vec![entry]);
    }
    ...
    fn a_duplicate_block_name_in_one_function_is_refused() {
        ...
        f.add_basic_block("entry".to_string(), &mut builder)
            .unwrap();
        ...
        assert_eq!(
            builder.funcs.get(f.raw()).unwrap().blocks.len(),
            1,
    ...
    fn the_same_block_name_in_another_function_is_fine() {
        ...
        assert_eq!(
            builder.blocks.get(in_f.raw()).unwrap().name,
            builder.blocks.get(in_g.raw()).unwrap().name
        );
    }
}
